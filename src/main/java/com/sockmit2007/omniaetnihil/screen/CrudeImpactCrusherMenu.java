...
public class CrudeImpactCrusherMenu extends AbstractContainerMenu {
    private final Container container;
    private final ContainerData data;
    protected final Level level;

    public CrudeImpactCrusherMenu(int containerId, Inventory playerInventory) {
        this(containerId, playerInventory, new SimpleContainer(3), new SimpleContainerData(3));
    }

    public CrudeImpactCrusherMenu(int containerId, Inventory playerInventory, Container container, ContainerData data) {
        super(OmniaEtNihil.CRUDE_IMPACT_CRUSHER_MENU.get(), containerId);
    ...
    public float getEnergyLevel() {
        ...
        int j = CrudeImpactCrusherBlockEntity.MAX_ENERGY;
        return j != 0 && i != 0 ? Mth.clamp((float) i / (float) j, 0.0F, 1.0F) : 0.0F;
    }
